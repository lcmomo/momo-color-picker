import { DateMode, DisabledDateFn, DisabledTimeConfig, DisabledTimeFn } from "../type";
import { CandyDate } from "./candy-date";

export const PREFIX_CLASS = 'ngx-mobile-picker';


export const InputFormats: { [key in DateMode]?: string } = {
  year: 'yyyy',
  month: 'yyyy-MM',
  quarter: 'yyyy-[Q]Q',
  week: 'yyyy-ww', // Format for week
  date: 'yyyy-MM-dd',
};

export function getInputFormat(mode: DateMode, showTime = false): string {
  if (mode === 'date' && showTime) return 'yyyy-MM-dd HH:mm:ss';
  return InputFormats[mode] as string ;
}

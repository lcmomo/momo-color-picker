...
import { NZ_DATE_CONFIG, NgTimeParser, NzDateConfig, WeekDayIndex, mergeDateConfig } from './date-config';
import { NzI18nService } from './nz-i18n.service';

export function DATE_HELPER_SERVICE_FACTORY(): DateHelperService {
  const i18n = inject(NzI18nService);
  const config = inject(NZ_DATE_CONFIG, InjectFlags.Optional);
...
export class DateHelperByDatePipe extends DateHelperService {
  ...
  //   // 使用 formatDate 处理除季度外的其他部分，这里先移除季度相关的格式字符
  //   let formattedBase = formatDate(date, formatStr.replace(/\[Q\]Q/g, ''), this.i18n.getLocaleId());
  
  //   // 现在调用 replaceQuarter 方法来插入正确的季度信息
  //   formattedBase = this.replaceQuarter(formattedBase, date);

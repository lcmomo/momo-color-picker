...
import { CompatibleDate, DateMode, RangePartType } from './type/index';

@Injectable({
  providedIn: 'root'
})
export class NgxDatePickerMobileService implements OnDestroy {
  ...
  setActiveDate(value: CompatibleValue, hasTimePicker: boolean = false, mode: NormalizedMode = 'month'): void {
    const parentPanels: { [key in DateMode]?: NormalizedMode } = {
      date: 'month',
      month: 'year',
      quarter: 'year',
      year: 'decade'
